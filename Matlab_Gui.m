...
% handles    structure with handles and user data (see GUIDATA)
  ...
  hw_status_msg=receive(handles.hw_status,10); % get calibrationNeeded flag 
...
function learningModeButton_Callback(hObject, eventdata, handles)
    ...
    if (Learning_mode_state_msg.Data==1)
          set_learningModeMsg(handles.learning_mode_client,handles.learning_mode_req,0,logs,handles) ;     
    else 
       set_learningModeMsg(handles.learning_mode_client,handles.learning_mode_req,1,logs,handles) ;   
    end
...
function trajectoryButton_Callback(hObject, eventdata, handles)
    ...
    [jointStateMsg,time]=get_JointState(handles,thero_trajectory_data,handles.jointState)
